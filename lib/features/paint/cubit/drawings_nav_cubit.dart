...
class DrawingsCubit extends Cubit<DrawingsState> {
  ...
  Future<void> addDrawing(DrawingModel d) async {
    ...
    if (exists) {
      ...
      debugPrint(
        'DrawingsCubit.addDrawing -> updated existing drawing id=${d.id}',
      );
    } else {
      await repository.add(d);
      debugPrint('DrawingsCubit.addDrawing -> added new drawing id=${d.id}');
    }
    await loadDrawings(); // يعيد تحميل الحالة من التخزين
  }
