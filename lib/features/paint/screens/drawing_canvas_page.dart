...
class DrawingCanvasPage extends StatelessWidget {
  ...
  final DrawingRepository repository; // ✅ أضف ده

  const DrawingCanvasPage({
    super.key,
    required this.projects,
    required this.onSave,
    required this.repository, // ✅ استقبله
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (_) {
        final cubit = DrawingCanvasCubit(
          projects: projects,
          existingDrawing: existingDrawing,
          repository: repository, // ✅ استعمله مباشرة
        );
        if (existingDrawing != null) {
          cubit.loadImagesForDrawing(existingDrawing!);
        }
        return cubit;
      },
      child: Scaffold(
              ...
              builder: (context, state) {
                return IconButton(
                  ...
                  onPressed: () {
                    ...
                    final existingSavedId = cubit.state.drawings.isNotEmpty
                        ? cubit.state.drawings.last.id
                        : null;
                    ...
                    debugPrint(
                      'Saving drawing, images in cubit.state.images: ${cubit.state.images.map((i) => i.toMap()).toList()}',
                    );
