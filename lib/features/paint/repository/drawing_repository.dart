...
class DrawingRepository {
  ...
  Future<List<DrawingModel>> getAll() async {
    final list = await drawingsPrefs.getModels(
      CacheKeys.drawingsKey,
      (j) => DrawingModel.fromMap(j),
    );
    debugPrint('Repository.getAll -> found ${list.length} drawings');
    for (var d in list) {
      debugPrint('-> loaded drawing id=${d.id}, images=${d.images.length}');
    }
    return list;
  }
  ...
  Future<void> saveAll(List<DrawingModel> drawings) async {
    debugPrint('Saving all drawings count=${drawings.length}');
    for (var d in drawings) {
      debugPrint(
        '-> save drawing id=${d.id}, images=${d.images.length}, imagesPaths=${d.images.map((i) => i.imagePath).toList()}',
      );
    }
    await drawingsPrefs.saveModels(
      CacheKeys.drawingsKey,
