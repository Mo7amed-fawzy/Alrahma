...
class ActiveClientsSection extends StatelessWidget {
  const ActiveClientsSection({super.key});
        ...
        children: activeClients.map((it) {
          ...
          return Card(
            ...
            child: ListTile(
              ...
              subtitle: Text(
                'مشاريع: $projectCount • جارٍ: ${hasOpen ? "نعم" : "لا"} • دفعات حديثة: ${recentPayment ? "نعم" : "لا"}',
                style: CustomTextStyles.cairoRegular14,
              ),
              trailing: IconButton(
                icon: const Icon(
    ...
    return _buildSectionWrapper(
      ...
      child: Column(
        children: recent.map((p) {
          ...
          // ✅ هنا الحسابات الخاصة بالـ history
          final paidFromHistory = p.history.fold<double>(
            0.0,
            (s, h) => s + h.amountPaid,
          );
          final displayedPaid = paidFromHistory > 0
              ? paidFromHistory
              : p.amountPaid;
          ...
          return Card(
            ...
            child: ListTile(
              ...
              subtitle: Text(
                // ✅ استخدمت displayedPaid بدل p.amountPaid هنا
                'المدفوع: ${displayedPaid.toStringAsFixed(2)} • $dateStr',
                style: CustomTextStyles.cairoRegular14,
              ),
              trailing: Text(
